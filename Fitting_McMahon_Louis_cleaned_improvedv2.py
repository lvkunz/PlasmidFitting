...
def chi2_one_form(data, sd, n, model):
    ...
    for i in range(len(data)):
        if sd[i] != 0:  # Additional check to avoid division by zero
            ...
            chi_squared += ((data[i] - model[i]) / sd[i]) ** 2
            # chi_squared += ((data[i] - model[i])) ** 2
...
def objective(params, dataS, dataC, dataL, sdS, sdC, sdL, n, S0, C0, rho, d):
    ...
    modelS = model_S(d, S0, C0, rho, b_S, b_D)
    modelC = model_C(d, S0, C0, rho, b_S, b_D)
    modelL = model_L(d, S0, C0, rho, b_S, b_D)

    # Calculate the chi-squared value
    chi_squared = chi2_all_forms(dataS, dataC, dataL, sdS, sdC, sdL, n, modelS, modelC, modelL)
...
def calculate_negative_log_likelihood(params, dataS, dataC, dataL, sdS, sdC, sdL, n, S0, C0, rho, d):
    ...
    print('params : ', b_S, b_D)

    modelS = model_S(d, S0, C0, rho, b_S, b_D)
    modelC = model_C(d, S0, C0, rho, b_S, b_D)
    modelL = model_L(d, S0, C0, rho, b_S, b_D)

    log_likelihood_S = np.log(norm.pdf(dataS, loc=modelS, scale=sdS))
    log_likelihood_C = np.log(norm.pdf(dataC, loc=modelC, scale=sdC))
    log_likelihood_L = np.log(norm.pdf(dataL, loc=modelL, scale=sdL))

    # Replace nan and inf values with 0
    log_likelihood_S = np.nan_to_num(log_likelihood_S, nan=0, posinf=1e3, neginf=-1e3)
    log_likelihood_C = np.nan_to_num(log_likelihood_C, nan=0, posinf=1e3, neginf=-1e3)
    log_likelihood_L = np.nan_to_num(log_likelihood_L, nan=0, posinf=1e3, neginf=-1e3)

    log_likelihood_S = d*log_likelihood_S
    log_likelihood_C = d*log_likelihood_C
    log_likelihood_L = d*log_likelihood_L

    log_likelihood = np.sum(log_likelihood_S)+np.sum(log_likelihood_C)+np.sum(log_likelihood_L)
...
def bootstrap_covariance_matrix(objective, initial_params, bounds, args, num_iterations, method = None):
    parameter_estimates = []
    print("Bootstrapping...")

    bootstrap_beta_S = np.zeros(num_iterations)
    bootstrap_beta_D = np.zeros(num_iterations)

    if method == None:
        print("CAREFUL: No method chosen, using default")
    for _ in range(num_iterations):
        if _ % 1000 == 0:
            print(_)
        resampled_args = resample_data(*args)

        resampled_initial_params = np.random.normal(initial_params, 5.0)
        #resampled_initial_params = initial_params
        # Perform optimization
        result = minimize(
            objective,
            resampled_initial_params,
            args= resampled_args,
            bounds=bounds,
            method=method,
        )
        ...
        bootstrap_beta_S[_], bootstrap_beta_D[_] = result.x
        parameter_estimates.append(result.x)

    plt.figure(figsize=(12, 6))

    plt.subplot(1, 2, 1)
    plt.hist(bootstrap_beta_S, bins=30, edgecolor='black')
    plt.title('Distribution of Bootstrap Estimates for beta_S')

    plt.subplot(1, 2, 2)
    plt.hist(bootstrap_beta_D, bins=30, edgecolor='black')
    plt.title('Distribution of Bootstrap Estimates for beta_D')

    plt.show()
    # Compute covariance matrix from the distribution of parameter estimates
    covariance_matrix = np.cov(np.array(parameter_estimates).T)
...
def MAGIC_Plasmid_CurveFitting_NCh_FixedS0C0rho(condition, beam, path_to_folder, delimiter = ";" ):
    ...
    initial_params = [0.3, 0.005]
    if condition == '21%':
        initial_params = [0.3, 0.005]
    elif condition == '1%':
        initial_params = [0.2, 0.003]
    elif condition == 'DMSO':
        initial_params = [0.02, 0.0003]
    ...
    print("SSB CONV:", optimized_params_CONV[0], '+-', parameter_standard_errors_CONV[0])
    ...
    # Extract the optimized parameters
    print("SSB FLASH:", optimized_params_FLASH[0], '+-', parameter_standard_errors_FLASH[0])
    ...
    # do a t-test to see if the difference in SSB and DSB is significant
    # t-test for SSB
    ttest_SSB = (optimized_params_CONV[0] - optimized_params_FLASH[0]) / np.sqrt(parameter_standard_errors_CONV[0]**2 + parameter_standard_errors_FLASH[0]**2)
    print("t-test SSB:", ttest_SSB)
    df = len(CONV_fracS) + len(FLASH_fracS) - 2
    pval_SSB = 2*t.sf(np.abs(ttest_SSB), df)
    if pval_SSB < 0.05:
        print("p-value SSB:", pval_SSB, "Significant difference")
    else:
        print("p-value SSB:", pval_SSB, "No significant difference")
    # t-test for DSB
    ttest_DSB = (optimized_params_CONV[1] - optimized_params_FLASH[1]) / np.sqrt(parameter_standard_errors_CONV[1]**2 + parameter_standard_errors_FLASH[1]**2)
    print("t-test DSB:", ttest_DSB)
    pval_DSB = 2*t.sf(np.abs(ttest_DSB), df)
    if pval_DSB < 0.05:
        print("p-value DSB:", pval_DSB, "Significant difference")
    else:
        print("p-value DSB:", pval_DSB, "No significant difference")


    # # Output file with fitting parameters
    with open(condition + "-" + beam + '.txt', 'w') as file:
        file.write("============== CONV =============================")
        file.write("\nSSB CONV:" + str(optimized_params_CONV[0]) + '+-' + str(parameter_standard_errors_CONV[0]))
        ...
        file.write("\n ============== UHDR =============================")
        file.write("\nSSB FLASH:" + str(optimized_params_FLASH[0]) + '+-' + str(parameter_standard_errors_FLASH[0]))
    ...
    plt.plot(CONV_d_fit, model_L(CONV_d_fit, S0_CONV, C0_CONV , rho, optimized_params_CONV[0], optimized_params_CONV[1]),
    ...
    plt.plot(FLASH_d_fit, model_L(FLASH_d_fit, S0_FLASH, C0_FLASH , rho, optimized_params_FLASH[0], optimized_params_FLASH[1]),
